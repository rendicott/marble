...
func allSpecs() []model.ToolSpec {
	return []model.ToolSpec{
		...
		spec("model_add", "Create a Settings → Models catalog entry. Prefer web research (mcp_tavily / web_fetch) for OpenAI-compatible base_url, context_limit, max_output, and capabilities. NEVER put API secrets in arguments — only api_key_env var NAMES (e.g. GEMINI_API_KEY). Operator puts KEY=secret in $MEMORY/env (Settings → Secrets). Gemini OpenAI path: https://generativelanguage.googleapis.com/v1beta/openai (not /interactions). Empty base_url inherits process --base-url. Empty api_key_env inherits process key; none = no Authorization.",
			map[string]interface{}{
				...
				"properties": map[string]interface{}{
					...
					"kind":               map[string]interface{}{"type": "string", "description": "chat (default) or image. kind=image is gpt-image-* via generate_image, not a session model"},
		...
		spec("model_update", "Update an existing catalog model by id (not process). Same fields as model_add; omitted numeric zeros may keep prior values when the server merges. Never put secrets in api_key_env.",
			map[string]interface{}{
				...
				"properties": map[string]interface{}{
					...
					"kind":               map[string]interface{}{"type": "string", "description": "chat|image; omit to keep the existing kind"},
		...
		spec("manage_sinks", "Create and manage turn sinks (ADR-0028): mirror finished turns to Orb, Slack, ntfy, Discord, webhook, or stdout. action=list|get|create|update|delete|test|set_override|pause_all|resume_all|set_base|env_names. NEVER put secrets in arguments — only secret_env env-var NAMES (e.g. ORB_AK). Operator stores KEY=secret in $MEMORY/env (Settings → Secrets). create requires id+type; orb needs topic_id; slack/discord need secret_env (webhook URL env name); ntfy needs topic; webhook needs url+template (defaults supplied). set_override is THIS session only (override=inherit|on|off).",
			map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"action":         map[string]interface{}{"type": "string", "description": "list|get|create|update|delete|test|set_override|pause_all|resume_all|set_base|env_names"},
					"id":             map[string]interface{}{"type": "string", "description": "sink id slug, e.g. orb"},
					"type":           map[string]interface{}{"type": "string", "description": "orb|webhook|slack|discord|ntfy|stdout"},
					"enabled":        map[string]interface{}{"type": "boolean", "description": "global default; per-session override can still force on/off"},
					"deep_link_base": map[string]interface{}{"type": "string", "description": "per-sink origin override (https://host:8080)"},
					"secret_env":     map[string]interface{}{"type": "string", "description": "env var NAME only, never the secret"},
					"topic_id":       map[string]interface{}{"type": "string", "description": "orb topic t_…"},
					"api_base":       map[string]interface{}{"type": "string", "description": "orb API origin; default https://api.dev.orbnet.app"},
					"title_prefix":   map[string]interface{}{"type": "string", "description": "orb banner prefix, default 'marble: '"},
					"url":            map[string]interface{}{"type": "string", "description": "webhook URL"},
					"method":         map[string]interface{}{"type": "string", "description": "POST|PUT|PATCH"},
					"headers":        map[string]interface{}{"type": "object", "additionalProperties": map[string]interface{}{"type": "string"}},
					"template":       map[string]interface{}{"type": "string", "description": "webhook Go text/template body over TurnEvent"},
					"channel":        map[string]interface{}{"type": "string"},
					"username":       map[string]interface{}{"type": "string"},
					"icon_emoji":     map[string]interface{}{"type": "string"},
					"avatar_url":     map[string]interface{}{"type": "string"},
					"server":         map[string]interface{}{"type": "string", "description": "ntfy server, default https://ntfy.sh"},
					"topic":          map[string]interface{}{"type": "string", "description": "ntfy topic"},
					"priority":       map[string]interface{}{"type": "integer", "description": "ntfy 1–5"},
					"format":         map[string]interface{}{"type": "string", "description": "stdout plain|json"},
					"filters": map[string]interface{}{
						"type": "object",
						"properties": map[string]interface{}{
							"kinds":            map[string]interface{}{"type": "array", "items": map[string]interface{}{"type": "string"}, "description": "complete|error|stop|cron|continuation"},
							"skip_empty":       map[string]interface{}{"type": "boolean"},
							"skip_cron":        map[string]interface{}{"type": "boolean"},
							"only_sessions":    map[string]interface{}{"type": "array", "items": map[string]interface{}{"type": "string"}},
							"skip_sessions":    map[string]interface{}{"type": "array", "items": map[string]interface{}{"type": "string"}},
							"min_chars":        map[string]interface{}{"type": "integer"},
							"min_interval_sec": map[string]interface{}{"type": "integer"},
						},
					},
					"override":              map[string]interface{}{"type": "string", "description": "set_override: inherit|on|off"},
					"global_deep_link_base": map[string]interface{}{"type": "string", "description": "set_base: default origin for /s/{id} links"},
				},
				"required": []string{"action"},
			}),
		...
		spec("generate_image", "Raster image generation via an OpenAI-compatible Images API (gpt-image-2.5-sunburst/flare); writes into the workspace, attaches to chat, optional vectorize for true SVG. Use a kind=image catalog row — these models are not session chat models.",
			map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"prompt":        map[string]interface{}{"type": "string", "description": "What to draw"},
					"path":          map[string]interface{}{"type": "string", "description": "Workspace-relative output file; default generated/image-<unix-ts>.png"},
					"model_id":      map[string]interface{}{"type": "string", "description": "Catalog id of a kind=image row; default is the first enabled image model"},
					"size":          map[string]interface{}{"type": "string", "description": "Default 1024x1024 (also 1024x1536, 1536x1024, auto)"},
					"quality":       map[string]interface{}{"type": "string", "description": "low|medium|high|xhigh|max|auto; default medium"},
					"background":    map[string]interface{}{"type": "string", "description": "Default transparent"},
					"output_format": map[string]interface{}{"type": "string", "description": "Default png"},
					"n":             map[string]interface{}{"type": "integer", "description": "Default 1, max 4. Extra files are suffixed -2, -3, …"},
					"vectorize":     map[string]interface{}{"type": "boolean", "description": "Also write a sibling .svg via vtracer. Default false. gpt-image models emit raster only."},
					"attach":        map[string]interface{}{"type": "boolean", "description": "Stage a durable chat attachment. Default true."},
				},
				"required": []string{"prompt"},
			}),
		spec("attach_from_url", "HTTP(S) GET image URL(s) and stage durable chat attachments (png/jpeg/webp/gif). After search returns image URLs, call this then cite attachment_id. Do not shell-curl images unless this tool fails. Pass url (single) or urls (max 4). SVG rejected. LAN allowed; cloud metadata blocked. 8 MiB max. Provenance stored; not a license clearance. Optional name/alt/credit/license/source_page.",
			map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"url":         map[string]interface{}{"type": "string", "description": "Single image URL (http/https)"},
					"urls":        map[string]interface{}{"type": "array", "items": map[string]interface{}{"type": "string"}, "description": "Batch, max 4; do not also pass url"},
					"name":        map[string]interface{}{"type": "string", "description": "Display name for a single URL"},
					"alt":         map[string]interface{}{"type": "string"},
					"source_page": map[string]interface{}{"type": "string", "description": "Page where the image was found"},
					"credit":      map[string]interface{}{"type": "string"},
					"license":     map[string]interface{}{"type": "string", "description": "Claimed license text; not validated"},
				},
			}),
		spec("message_attach", "Attach an image or basic document to the chat transcript (durable chip the operator can download). Prefer this over attach_file when the user wants files in the chat. Images: png/jpeg/webp/gif. Docs: txt/md/csv/json/html/svg (SVG is a document, not an inline image). No audio/PDF. Prefer workspace path. For remote image URLs use attach_from_url instead of downloading then attaching. Not re-injected as a full tool result.",
			map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
		...
		spec("web_fetch", "HTTP(S) GET a URL for deeper analysis. HTML/text is returned as markdown; JSON is returned raw. Prefer after web search (e.g. mcp_tavily_tavily_search) returns real URLs — do not invent URLs. For image URLs use attach_from_url instead. Use Tavily extract/crawl only when fetch fails or multi-page crawl is needed. Prefer over shell curl. LAN/private hosts allowed; cloud metadata blocked.",
			map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
